...
<ul class="pagination err_rmv6 main_pagi6">
	<?php
        if(!empty($total_page)){
            ...
            if(empty($page_id)){
                ...
                if($total_page > 1){
                    $next = site_url('welcome/table_filter_saved_seller/2');
                }
            }else{ 
                if($page_id == 1){
                    ...
                    if($total_page == $page_id){
                       $nxt_page = "javascript:void(0)";
                        ...
                        $nxt_page = $page_id+1;
                        $nxt_page = site_url('welcome/table_filter_saved_seller/'. $nxt_page);
                    }
                    $next = $nxt_page;
                }else{
                    if($page_id >= "2"){
                        $prv_pg = $page_id-1;
                        $prev = site_url('welcome/table_filter_saved_seller/'.$prv_pg);    
                    }else{
                        $prev = site_url('welcome/table_filter_saved_seller/');    
                    }
                    if($total_page > 1){ 
                        if($total_page == $page_id){
                           $nxt_page = "javascript:void(0)";
    ...
    <?php
            for($i =1; $i <= $total_page;$i++){
                if($i == 1){
                    $page_link = site_url('welcome/table_filter_saved_seller/');
                }else{
                    $page_link = site_url('welcome/table_filter_saved_seller/'. $i); 
                }
